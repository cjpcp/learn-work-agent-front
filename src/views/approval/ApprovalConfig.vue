  ...
  <div class="approval-config-container">
    ...
    >
      <a-form :model="stepForm" layout="vertical">
        ...
        <div class="form-row">
          <a-form-item label="审批类型"><a-select v-model:value="stepForm.approvalType" :get-popup-container="trigger => trigger.parentNode"><a-select-option value="SINGLE">单人审批</a-select-option><a-select-option value="MULTIPLE">多人审批</a-select-option></a-select></a-form-item>
        ...
        <div class="form-row">
          <a-form-item label="审批角色"><a-select v-model:value="stepForm.approverRole" :loading="loadingRoles" :get-popup-container="trigger => trigger.parentNode" @change="handleRoleChange"><a-select-option v-for="role in approverRoles" :key="role.code" :value="role.code">{{ role.name }}</a-select-option></a-select></a-form-item>
          <a-form-item label="是否必须通过"><a-switch v-model:checked="stepForm.mustPass" /></a-form-item>
        </div>

        <a-form-item v-if="stepForm.assignMode === 'ROLE'" label="目标角色">
          <a-select v-model:value="stepForm.roleId" placeholder="请选择角色" :loading="loadingRoles" :get-popup-container="trigger => trigger.parentNode">
            <a-select-option v-for="role in approverRoles" :key="role.code" :value="role.code">{{ role.name }}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        <template v-if="stepForm.assignMode === 'USER'">
          <div class="filter-grid">
            <a-form-item label="角色筛选">
              <a-select v-model:value="filters.roleId" placeholder="选择角色后仅列举该角色用户" allow-clear :loading="loadingRoles" :get-popup-container="trigger => trigger.parentNode">
                <a-select-option v-for="role in approverRoles" :key="role.id" :value="role.id">{{ role.name }}</a-select-option>
              </a-select>
            </a-form-item>
            ...
            <a-form-item label="账号状态">
              <a-select v-model:value="filters.status" placeholder="全部状态" allow-clear :get-popup-container="trigger => trigger.parentNode">
...
const stepForm = reactive({ stepName: '', stepOrder: 1, approvalType: 'SINGLE', approverRole: '', approverUserId: undefined as number | undefined, mustPass: true, assignMode: 'USER', roleId: undefined as string | undefined })
...
const handleAddStep = () => {
  ...
  Object.assign(stepForm, { stepName: '', stepOrder: steps.value.length + 1, approvalType: 'SINGLE', approverRole: '', approverUserId: undefined, mustPass: true, assignMode: 'USER', roleId: undefined })
  resetFilters()
...
const handleSaveStep = async () => {
  ...
  try {
    const stepData = {
       ...
       approverUserId: stepForm.assignMode === 'USER' ? (selectedRowKeys.value.length > 0 ? selectedRowKeys.value[0] : undefined) : undefined,
